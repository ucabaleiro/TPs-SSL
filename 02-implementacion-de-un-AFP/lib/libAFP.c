...
unsigned int sintacticamenteCorrecta ( char expresion[] )
{
    ...
    while ( expresion[i] != '\n') 
    {
        if (expresion[i] != ' ') // Ignora los espacios
        {   //Obtiene un nuevo movimiento de la tabla a partir del estado actual del automata, de la pila y del ultimo caracter leido
